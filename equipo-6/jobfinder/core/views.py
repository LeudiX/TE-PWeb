        ...
        if user_form.is_valid():
            ...
            if user_type == 'company':
                ...
                if company_form.is_valid():
                    ...
                    # Si hay error en el formulario de empresa, eliminar el usuario creado
                    user.delete()
...
def create_job_offer(request):
    # Verificar que el usuario tenga un perfil de empresa
    try:
...
def apply_to_offer(request, pk):
    ...
    if offer.is_expired:
        messages.error(request, 'Esta oferta ha expirado.')
        return redirect('job_offer_detail', pk=pk)
...
def application_list(request, offer_pk):
    ...
    applications = Application.objects.filter(job_offer=offer)
