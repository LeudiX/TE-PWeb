export function validarNombreCategoria(valor) {
	...
	if (!regex.test(nombre)) {
		...
		if (/[^\p{L} ]/u.test(nombre)) return 'No se permiten números ni caracteres especiales, solo letras y espacios.';

		// 3) Si hay mayúsculas en el resto de la cadena
		//const resto = nombre.slice(1);
		//if (/\p{Lu}/u.test(resto)) return 'Sólo la primera letra puede ser mayúscula.';
