...
document.addEventListener("DOMContentLoaded", () => {
  ...
  selectAllCheckbox.addEventListener("change", () => {
    const pageCheckboxes = document.querySelectorAll(
      "#productTableBody .product-checkbox"
    );
    ...
    if (selectAllCheckbox.checked && pageCheckboxes.length > 0) {
      setTimeout(() => {
        ...
        if (confirmed) {
          const indices = Array.from(pageCheckboxes)
  ...
  tableBody.addEventListener("click", (e) => {
    ...
    if (deleteBtn) {
      ...
      const confirmed = confirm(
        `¿Estás seguro de eliminar el producto "${products[idx].name}"?`
      );
      if (!confirmed) return;
  ...
  function toggleDeleteButton() {
    const checkedCount = document.querySelectorAll(
      "#productTableBody .product-checkbox:checked"
    ).length;
  ...
  deleteSelectedBtn.addEventListener("click", () => {
    const checkedBoxes = Array.from(
      document.querySelectorAll("#productTableBody .product-checkbox:checked")
    );
    ...
    const confirmed = confirm(
      `¿Estás seguro de eliminar ${checkedBoxes.length} producto(s)?`
    );
    if (!confirmed) return;
  ...
  return `
        <tr>
            ...
            <td>
                ...
                <a href="/templates/modificarProducto.html ?id=${product.id}" class="btn btn-warning btn-sm">
...
document.addEventListener("DOMContentLoaded", () => {
      ...
      setTimeout(() => {
        ...
        if (confirmed) {
          ...
          const maxPage = Math.max(
            1,
            Math.ceil(getDisplayList().length / rowsPerPage)
          );
          if (currentPage > maxPage) currentPage = maxPage;
          renderTable(currentPage);
          alert("Productos eliminados correctamente.");
  ...
  tableBody.addEventListener("click", (e) => {
    ...
    if (editBtn) {
      const idx = Number(editBtn.dataset.index);
      window.location.href = `/templates/modificarProducto.html?id=${idx}`;
      return;
    }
